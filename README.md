...
It's goal is to extend the Pivot table with additional the additional aggregation, therefore you'll need a pivot table (or if already exists, a column that contains the invoiceCount of each invoice you are trying to aggregate).
The program assumes you have the invoice data on Work1 and it's pivot on Work2, with column numbers being SPECIFIC to my use case, so please take this into consideration when refactoring and applying.
